      ...
      <div className="rounded-lg border border-border/40 bg-muted/20 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/30">
          <div className="flex items-center gap-2">
            <span className="size-2 rounded-full flex-shrink-0" style={{ backgroundColor: broker.color }} />
        ...
        <div className="flex flex-col gap-2 px-4 py-3">
          ...
          <div className="flex gap-2">
            ...
            {connected && (
              <Button
                size="sm"
                className={cn("flex-1 h-8 text-xs gap-1", isBeforeCutoff && "opacity-50 cursor-not-allowed")}
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="bottom" className="max-h-[90dvh] overflow-y-auto rounded-t-xl px-0">
          <SheetHeader className="px-4 pb-2">
            <SheetTitle className="text-base">Configure {broker.name}</SheetTitle>
          </SheetHeader>

          <div className="flex flex-col gap-4 px-4 pb-4">
            <div className="space-y-1.5">
              <Label htmlFor={`${broker.id}-api-key`} className="text-xs text-muted-foreground">API Key</Label>
              <Input
            ...
            <div className="space-y-1.5">
              <Label htmlFor={`${broker.id}-api-secret`} className="text-xs text-muted-foreground">API Secret</Label>
              <Input
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 rounded-md border border-border/40 bg-muted/30 px-3 h-9">
                <span className="flex-1 text-xs text-muted-foreground font-mono truncate">{redirectUrl}</span>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2 rounded-md border border-border/40 bg-muted/30 px-3 h-9">
                <span className="flex-1 text-xs text-muted-foreground font-mono truncate">{webhookUrl}</span>
                <CopyButton value={webhookUrl} />
              </div>
            </div>

            <Button className="w-full h-9 text-sm mt-1" onClick={handleSave} disabled={saving}>
