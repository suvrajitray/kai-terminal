...
const INDEX_TO_FEED: Record<string, keyof ReturnType<typeof useIndicesFeed>> = {
  NIFTY:     "nifty",
  BANKNIFTY: "bankNifty",
  SENSEX:    "sensex",
  FINNIFTY:  "finNifty",
...
function payoffAt(legs: Leg[], spot: number): number {
...
export function PayoffChartDialog({ open, onOpenChange, positions }: Props) {
  ...
  // Group legs by expiry, derive primary index — no live feed dependency
  ...
  // Spot price updates live with ticks — outside memo so it doesn't re-derive legs
