...
- **`ZerodhaTokenMappingProvider`** (`Options/`) — implements `ITokenMappingProvider`; returns `NativeContractKey` list (InstrumentToken + Segment + ExchangeToken + TradingSymbol) for all CE/PE contracts from the Kite CSV. Used by `CrossBrokerTokenMapper` in the Worker.
...
- `ToFeedTokens(brokerType, nativeTokens)` — Upstox: identity. Zerodha: `nativeToken → NSE_FO|{exchangeToken}`.
- `ToNativeToken(brokerType, feedToken)` — reverse direction.
