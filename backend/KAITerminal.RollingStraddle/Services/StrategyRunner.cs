...
internal sealed class StrategyRunner : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        ...
        {
            _logger.LogInformation("[CONFIG] Expiry {Msg} — resolving nearest upcoming expiry...",
                expiryMissing ? "not set" : $"{_cfg.Expiry} is in the past");
            DateOnly? resolved = await _feed.ResolveExpiryAsync(ct);
            if (resolved is null)
            {
                _logger.LogError("[CONFIG] Could not resolve expiry from API — set Strategy:Expiry manually and restart.");
                _lifetime.StopApplication();
                return;
            }
            _cfg.Expiry = resolved.Value.ToString("yyyy-MM-dd");
            _logger.LogInformation("[CONFIG] Resolved expiry: {Expiry}", _cfg.Expiry);
        }
                ...
                try
                {
                    if (state.ReEntryAfter.HasValue)
                    {
                        if (DateTimeOffset.UtcNow < state.ReEntryAfter.Value)
                        {
                            ...
                            if (NowIst() >= exitTime)
                            {
                                _logger.LogInformation("[EXIT ] Past exit time during re-entry cooldown — no new entry today");
                                _lifetime.StopApplication();
                                return;
                            }
                            await Delay(ct);
                    ...
                    if (spot <= 0)
                    {
                        _logger.LogWarning("[FEED ] Spot unavailable — retrying in {Ms}ms", _cfg.CheckIntervalMs);
                        await Delay(ct);
                        continue;
                    }
                    ...
                    switch (decision)
                    {
                        case StrategyDecision.WaitForEntry waitForEntry:
                            _logger.LogInformation(
                                "[IDLE ] Entry at {Time} — {Min}m {Sec}s remaining  |  Spot {Spot:F2}",
                                _cfg.EntryTime,
                                (int)waitForEntry.Remaining.TotalMinutes,
                                waitForEntry.Remaining.Seconds,
                                spot);
                            break;
                        ...
                        case StrategyDecision.Roll roll:
                            state = await HandleRollAsync(roll, spot, state, snapshot, ct);
                            break;

                        case StrategyDecision.HoldMaxRolls holdMaxRolls:
                            _logger.LogWarning(
                                "[ROLL ] Max rolls ({Max}) reached — holding  |  Spot {Spot:F2}  Move {Move:+0.00;-0.00}%",
                                _cfg.MaxRolls, spot, holdMaxRolls.MovePct);
                        ...
                        case StrategyDecision.Exit exit:
                            await HandleExitAsync(exit.Reason, state, snapshot, ct);
                            return;
                    }
                }
                catch (Exception ex) when (ex is not OperationCanceledException)
                {
                    _logger.LogError(ex, "[ERROR] Unhandled exception — retrying in {Ms}ms", _cfg.CheckIntervalMs);
                }
        ...
        catch (OperationCanceledException)
        {
            if (state.HasOpenLegs)
                ...
                _logger.LogInformation("[EXIT ] Shutdown — no open positions");
        }
        ...
        decimal spot, StrategyState current, CancellationToken ct)
    {
        ...
        if (_cfg.VixMaxThreshold > 0)
        {
            ...
            if (vix <= 0)
            {
                _logger.LogWarning("[VIX  ] Unavailable — blocking entry as precaution");
                return current;
            }
            if (vix > _cfg.VixMaxThreshold)
            {
                _logger.LogWarning("[VIX  ] {Vix:F1} exceeds threshold {Max} — skipping entry", vix, _cfg.VixMaxThreshold);
                return current;
            }
            vixOk = vix;
        }
        ...
        if (vixOk.HasValue)
            _logger.LogInformation("[VIX  ] {Vix:F1} — OK (threshold {Max})", vixOk.Value, _cfg.VixMaxThreshold);
        _logger.LogInformation("[ENTRY] Entry window open  |  Spot {Spot:F2}", spot);
        ...
        decimal spot, StrategyState current, CancellationToken ct)
    {
        ...
        if (_cfg.StrikeOffset == 0)
            _logger.LogInformation("[ENTRY] ATM {Strike}  |  Spot {Spot:F2}", atmStrike, spot);
        else
            _logger.LogInformation("[ENTRY] ATM {Atm}  CE@{Ce}  PE@{Pe}  |  Spot {Spot:F2}",
                atmStrike, ceStrike, peStrike, spot);

        _logger.LogInformation("[ENTRY] Placing {Type} — SELL CE {Ce}  SELL PE {Pe}  Qty {Qty}",
        ...
        _logger.LogInformation("[ENTRY] Orders sent — CE {CeId}  PE {PeId} — awaiting fills", ceOrderId, peOrderId);
    ...
    private async Task<StrategyState> HandleRollAsync(
        StrategyDecision.Roll roll, decimal spot, StrategyState state, MarketSnapshot snapshot, CancellationToken ct)
    {
        _logger.LogWarning(Sep);
        _logger.LogWarning(
            "  ROLL #{N}  |  Spot {Spot:F2}  Move {Move:+0.00;-0.00}%  from {Entry:F2}",
            roll.RollNumber, spot, roll.MovePct, state.EntrySpot);
        _logger.LogWarning(
        ...
        _logger.LogInformation("[ROLL ] Closing current legs...");
        ...
        {
            _logger.LogInformation("[ROLL ] Opening new {Type} at Spot {Spot:F2}",
    ...
    private async Task<StrategyState> CloseLegsAsync(StrategyState state, CancellationToken ct)
    {
        ...
        {
            _logger.LogInformation("[CLOSE] BUY CE {Token}  Qty {Qty}", ce.Token, ce.Qty);
            var id = await _executor.BuyMarketAsync(ce.Token, ce.Qty, ct);
            await _executor.WaitForFillAsync("CE", id, ct);
        }
        if (state.PeLeg is { } pe)
        {
            _logger.LogInformation("[CLOSE] BUY PE {Token}  Qty {Qty}", pe.Token, pe.Qty);
    ...
    {
        _logger.LogInformation("[EXIT ] {Reason}  |  P&L {Sign}₹{Pnl:N0}",
            reason, PnlSign(snapshot.Pnl), Math.Abs(snapshot.Pnl));

        _logger.LogInformation("[EXIT ] Cancelling pending orders...");
        await _executor.CancelAllPendingAsync(ct);

        _logger.LogInformation("[EXIT ] Closing all open legs...");
        ...
        _logger.LogInformation("  Exit reason  :  {Reason}", reason);
        _logger.LogInformation("  Total rolls  :  {Rolls}", state.RollCount);
        _logger.LogInformation("  Final P&L    :  {Sign}₹{Pnl:N0}", PnlSign(finalPnl), Math.Abs(finalPnl));
        _logger.LogInformation(Sep);
    ...
    {
        _logger.LogWarning("[EXIT ] Ctrl+C — checking broker positions before shutdown...");
        try
        {
            ...
            if (!ceOpen && !peOpen)
            {
                _logger.LogInformation("[EXIT ] Positions already closed — shutdown clean");
                return;
            }

            _logger.LogWarning(Sep);
            _logger.LogWarning("  GRACEFUL SHUTDOWN — closing open position(s)");
            _logger.LogWarning(Sep);
            ...
            _logger.LogInformation(Sep);
            _logger.LogInformation("  SHUTDOWN COMPLETE — positions closed");
            _logger.LogInformation(Sep);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[ERROR] Graceful shutdown failed — close positions manually in broker terminal");
        }
    }
    ...
    private void PrintBanner()
    {
        ...
        _logger.LogInformation(Sep);
        _logger.LogInformation("  KAI Terminal — {Strategy}", strategyName);
        _logger.LogInformation("  Underlying   :  {U}", _cfg.Underlying);
        _logger.LogInformation("  Expiry       :  {E}", _cfg.Expiry);
        _logger.LogInformation("  Size         :  {L} lots x {Ls} = {Total} units/leg",
            _cfg.Lots, _cfg.LotSize, _cfg.Lots * _cfg.LotSize);
        _logger.LogInformation("  Entry / Exit :  {Entry}  →  {Exit}", _cfg.EntryTime, _cfg.ExitTime);
        _logger.LogInformation("  Roll         :  {Roll}%  max {MaxRolls}x", _cfg.RollThresholdPct, _cfg.MaxRolls);
        _logger.LogInformation(
        ...
        _logger.LogInformation("  VIX filter   :  {Vix}",
            _cfg.VixMaxThreshold > 0 ? $"skip if VIX > {_cfg.VixMaxThreshold}" : "disabled");
        _logger.LogInformation("  Re-entry delay :  {Delay}",
